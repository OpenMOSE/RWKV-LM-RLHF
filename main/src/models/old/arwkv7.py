...
if 'xa07' in ModelGeneration:
    ...
    if 'flatriton' in RunningDevice or 'infctx' in TrainingType or ('state' in TrainingType and KernelMode == 1): #infctx or FLA Mode
        ...
        if 'cuda' in RunningDevice:
            print('ax070 Wind CUDA Kernel Mode')
            CHUNK_LEN = 16
            ...
            print('ax070 Wind Triton Kernel Mode')
    ...
    class ARWKV_Tmix_x070(nn.Module):
    ...
    class ARWKV_Tmix_x070_state(nn.Module):
        def __init__(self, args, layer_id):
            super().__init__()
            self.args = args
            self.layer_id = layer_id
        ...
        def forward(self, x, v_first,passthrough):
            ...
            if self.args.suffix_offset and self.args.prefix_tuning == 0:
                ...
                if self.args.suffix_offset:
                   # x = x  * (1 + self.time_offset_y)
                    xs = x.shape
                    #print(xs.shape)
                    x = x.view(B,T,-1) * (1 + self.time_offset_y)#.unsqueeze(1)
                    x = x.view(xs)
            ...
            if self.args.suffix_offset:
                x = x * (1+ self.time_offset)#.unsqueeze(1)
            return x, v_first
        

    class ARWKV_Tmix_x070_infctx(nn.Module):
        def __init__(self, args, layer_id):
            super().__init__()
            self.args = args
            self.layer_id = layer_id
