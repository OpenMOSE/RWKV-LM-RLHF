    ...
    def __init__(self, args, layer_id):
        ...
        with torch.no_grad():
            ...
            if LinearMode == 0:
                ...
                self.receptance = make_linear_att(C, C, bias=False,n_layer=self.layer_id)
                self.key = make_linear_att(C, C, bias=False,n_layer=self.layer_id)
                self.value = make_linear_att(C, C, bias=False,n_layer=self.layer_id)
                self.output = make_linear_att(C, C, bias=False,n_layer=self.layer_id)
...
class RWKV_CMix_x070(nn.Module):
    def __init__(self, args, layer_id):
        ...
        if LinearMode == 0:
            ...
            self.key = make_linear_ffn(args.n_embd, args.n_embd * 4, bias=False,n_layer=self.layer_id)
            self.value = make_linear_ffn(args.n_embd * 4, args.n_embd, bias=False,n_layer=self.layer_id)
