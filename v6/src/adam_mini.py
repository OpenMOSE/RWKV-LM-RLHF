    ...
    def __init__(
        ...
        for name, param in self.model.named_parameters():
            ...
            if param.requires_grad:
                ...
                if ("embed" in name or "wte" in name or "embd" in name or "emb" in name):
                    count_embd += 1

                if ("lm_head.weight" in name or "output.weight" in name or "head" in name):
                    count_output += 1

                if ("q_proj.weight" in name or "wq.weight" in name or "key" in name or "receptance" in name):
                    count_qk += 1
                ...
                if ("output" in name or "gate" in name or "time" in name or "ln" in name or "value" in name):
                    count_output += 1

                # if ("key.lora" in name or "receptance.lora" in name):
                #     count_qk += 1

                #     dic["parameter_per_head"] = self.n_feature * self.lora_r // self.n_head
                #     if (self.n_feature * self.lora_r % self.n_head) != 0:
                #         raise ValueError("'n_feature * n_feature' is not a multiple of  n_head ")
        ...
        self.embd_blocks = {
             "embed", "embd", "wte", "lm_head.weight",  "output.weight", "emb","output","gate","ln","time","value"
        }
        # Query and Keys, will  assign lrs by heads
        self.qk_blocks = {
            "k_proj.weight", "q_proj.weight", "wq.weight", "wk.weight", "key", "receptance"
        }
